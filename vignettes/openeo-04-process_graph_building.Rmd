...
The Graph structure is build from a single end node that is traversed upwards - meaning that the set arguments are parsed by their reference where the R6 environment structure comes into play. The `ProcessNode` is serialized where it is used as argument with a reference object (in JSON: `{"from_node":"some_node_id"}`). The actual ProcessNode needs to be serialized also, in fact as another process with the node id, the used process name and the arguments. Schematically the graph is defined like this:
...
The EVI user defined process would look like this:
```
{
...
process_viewer(p$load_collection())
describe_process("load_collection")


data_node = p$load_collection(id = "SENTINEL2_L2A_SENTINELHUB",temporal_extent = c("2022-05-01","2022-07-01"),bands = c("B04","B08"))
```

# POC: Use Case 3 RClient -> GEE back-end v0.4.1 status: not working, because missing process at gee back-end
...
data = graph$load_collection(id = graph$data$`COPERNICUS/S2`, spatial_extent = list(west = 16.1, east = 16.6, north = 48.6, south = 47.2), temporal_extent = list("2017-01-01", 
    "2017-02-01"), bands = list("B8"))
...
zonal_statistics = spectral_reduction %>% graph$aggregate_polygon(polygons = list(type = "Polygon", coordinates = list(c(16.138916, 48.320647), c(16.524124, 
    48.320647), c(16.524124, 48.1386), c(16.138916, 48.1386), c(16.138916, 48.320647))))
...
# Part 2: create a job
job_id = gee %>% create_job(graph = graph, title = "Zonal Statistics / Sentinel 2", description = "Compute time series of zonal (regional) statistics of Sentinel 2 imagery over user-specified polygons.")

...
task = pgb$collection$S2_L2A_T32TPS_20M %>% pgb$filter_bbox(extent = list(west = 10.99, east = 11.25, south = 46.59, north = 46.76)) %>% pgb$filter_daterange(extent = c("2016-01-01T00:00:00Z", 
    "2016-03-10T23:59:59Z")) %>% pgb$NDVI(red = "B04", nir = "B8A") %>% pgb$min_time() %>% pgb$stretch_colors(min = -1, max = 1)

raster = eurac %>% preview(task = task, format = "GTiff", output_file = "eurac_test.tif")

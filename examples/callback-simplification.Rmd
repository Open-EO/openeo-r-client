...
|0.4.x|1.0.0|
|-|-|
|process graph|user defined / custom process|
|openEO process| predefined process|
|callback / sub process graph |custom process|
|callback argument|custom process parameter|
|variable|parameter of a custom process|
...
Usually this behavior in R can be achieved by passing a function as parameter. In the openEO R client prior to 0.6.x callbacks had to be solely created as the openEO concept 'callback' by creating a sub graph and using the processes provided by the back-end. This will be still a viable option to do, if there are complex functions or if a particular back-end does not comply to the well defined common set of openEO processes.
...
However, this has limited the use and protability of this feature. Using the R call engine would make the feature protable and extensible. The challenge with this new approach was that we have to do the following:

* overload the primitve math operators, so that they work for the R6 classes `ProcessNode`, `Variable` and `callback-value`
* create a process graph automatically by evaluating the function with a `callback-value`

## callback-value

In the R client domain a `callback-value` is the defined parameter in the signature of the callback function. In the API it was called `callback-argument` in the versions < 1.0. In case of a temporal reduction the `callback-value` would be a time series or to be more technically an array or in R terms a numeric vector, when the graph it evaluated for processing at the back-end. But for the R-client this will not hold any real value. It refers simply to the parameter that has to be used in this context. In order to create the process graph for the computations at the back-end.

# Work Done

We overloaded many of the primitive operators in R. The result of those functions are `ProcessNodes` which are created by mapping the primitive functions to the equivalent openEO processes. This, however, requires back-ends to be fully compliant to the openEO processes in terms of those mathematical and logical operators. If the back-end is not compliant you can use the `ProcessNodes` created by a `Graph` from the `process_graph_builder()`. 
...
* if you want to use a single function in a callback and you have to set additional values, you have to wrap it into a unnamed function, otherwise the function is evaluated instead of passed on as a function definition

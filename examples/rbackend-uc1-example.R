...
graph = pgb$collection$sentinel2_subset %>%
  pgb$filter_daterange(extent = c("2017-04-01T00:00:00Z", "2017-05-31T00:00:00Z")) %>% 
  pgb$filter_bbox(extent = list(west=699960,south=7897000,east=702960,north=7900000,crs="EPSG:32734")) %>%
...
service_id = con %>% defineService(type = "wms",
                                   process_graph = graph,
                                   title="Minimum NDIV April/Mai 2017",
                                   enabled = TRUE,
                                   plan = "free",
                                   parameters = list(version="1.3.0"))

...
task = collection("s2a_prd_msil1c", "product_id") %>% 
  process("filter_bbox", prior.name = "imagery", left=652000,right=5161000,bottom=5181000,top=672000,srs="EPSG:32632") %>% 
  process("filter_daterange", prior.name="imagery", from= "2017-01-01", to="2017-01-08" ) %>% 
  process("NDVI", prior.name = "imagery", red="B04",nir="B08") %>% 
